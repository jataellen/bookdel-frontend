...
export default {
  ...
  computed: {
    ...
    pages() {
      let filteredItems = this.items.filter(item => item.title.toLowerCase().includes(this.searchTerm.toLowerCase()));
      if (this.filterByPasses) {
        filteredItems = filteredItems.filter(item => item.passes);
      }
